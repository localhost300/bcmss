...
const SessionDeleteForm = ({ id, onSuccess }: SessionDeleteFormProps) => {
  ...
  if (!sessionId) {
    return (
      <div className="p-4">
        <p className="text-sm text-red-500">Session id is missing.</p>
      </div>
    );
  }
  ...
  return (
    <form className="p-4 flex flex-col gap-4" onSubmit={(event) => event.preventDefault()}>
      <span className="text-center font-medium">
        This will permanently remove the academic session. Continue?
      </span>
...
export default SessionDeleteForm;
